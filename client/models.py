...
class bannerSlider(models.Model):
    ...
    name_arabic = models.CharField(max_length=30)
...
class serviceCategory(models.Model):
    ...
    title_arabic = models.CharField(max_length=200)
    image = models.ImageField(verbose_name='cat_Image',upload_to='categoryService_images/en/',blank=True)
    image_arabic = models.ImageField(verbose_name='cat_Image_arabic',upload_to='categoryService_images/ar/',blank=True)
    def __str__(self):
        return self.title
...
class subServiceCategory(models.Model):
    ...
    image = models.ImageField(verbose_name='item_image',upload_to='subCategoryService_images/en/',blank=True)
...
class service(models.Model):
    ...
    image = models.ImageField(verbose_name='item_image',upload_to='service_images/en/',help_text='this is the image for service to be viewed on order page',blank=True)
    image_arabic = models.ImageField(verbose_name='item_image_arabic',upload_to='service_images/ar/',help_text='this is the image for service to be viewed on order page',blank=True)
    minimum_cost = models.DecimalField(max_digits=10 , blank=True,null =True,decimal_places=2)
...
class warninig_text(models.Model):
    text = models.TextField(max_length=300,help_text='this is warning text for specific service, enter just single warning')
    def __str__(self):
        return '{}'.format(self.text)

class warninig(models.Model):
    service = models.ForeignKey(service,on_delete=models.CASCADE , help_text = 'associated service for this warning')
...
class order(models.Model):
    ...
    (COMPLETED,'Completed'),(UNCERTAIN,'Uncertain'),(SUSPENDED,'Suspended')
    ,(INPROGRESS,'InProgress'))
    ...
    serviceProvider = models.ForeignKey('provider.provider',on_delete=models.CASCADE,blank=True,null=True)
    service_status = models.CharField(max_length=3,choices = SERVICE_STATUS_CHOICES,default=UNCERTAIN)
    cost = models.DecimalField(max_digits=10 , blank=True,null =True,decimal_places=2)
    paymentMethod = models.CharField(max_length=3,choices=SERVICE_PAMENT_METHOD_CHOICES,default=ONLINE)
    def __str__(self):
        return self.service
